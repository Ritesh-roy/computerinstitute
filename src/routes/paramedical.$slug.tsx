...
export const Route = createFileRoute("/paramedical/$slug")({
  loader: ({ params }) => {
    const course = getCourseById(params.slug);
    if (!course || course.category !== "paramedical") throw notFound();
    return { course };
  },
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          ...
          { name: "description", content: `${loaderData.course.title} (${loaderData.course.duration}) at SK Institute — clinical training included.` },
      ...
      : [],
    links: loaderData
      ? [{ rel: "canonical", href: `https://sk-institute-quest.lovable.app/paramedical/${loaderData.course.id}` }]
      : [],
    scripts: loaderData
      ? [{
